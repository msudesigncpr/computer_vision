...
# cpr.process_petri_dish_image(image_folder_path='.\\ibs_ecoli\\altered\\', good_colony_coord_output_path='./good_colony_coords/', raw_yolo_dump_path=None, binary_save_folder_path='./binary_images/', model_sensitivity=0.01)


# cpr.create_metadata(image_folder_path='.\\ibs_ecoli\\altered\\', colony_coords_folder_path='.\good_colony_coords\\', create_petri_dish_view=True, create_colony_view= True)
...
count = 0
for file in os.listdir('.\\good_colony_coords\\'):
    print(file)
    # count how many lines are in each file
    with open('.\\good_colony_coords\\' + file, 'r') as f:
        for line in f:
            count += 1
